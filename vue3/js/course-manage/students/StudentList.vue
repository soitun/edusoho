  ...
  <ant-config-provider>
    <div class="flex flex-col grow shrink-0 justify-start items-start self-stretch gap-20 bg-white min-h-800">
      <div class="flex w-full justify-between items-center">
        ...
        <div v-if="isNormalCourse && isEnableAddAndRemove && ['published', 'unpublished'].includes(courseStatus)"
      ...
      <div class="flex flex-col w-full items-start gap-20">
        ...
        <div class="flex flex-col items-start self-stretch gap-20">
          ...
          <div v-show="students.length > 0 && isNormalCourse" class="flex items-start gap-20">
            <button v-show="isEnableAddAndRemove" class="student-operate-button" @click="onBatchRemove">批量移除
            </button>
          ...
          >
            ...
            <template #bodyCell="{ column, record }">
                ...
                <div class="flex items-center gap-12 shrink-0">
                  ...
                  <div class="flex flex-col">
                    <a-tooltip placement="top">
                      ...
                      <div @click="open(`/user/${record.user.uuid}`)"
                           class="w-fit max-w-100 truncate text-14 text-[#1D2129] cursor-pointer hover:text-[--primary-color]">
              ...
              <template v-else-if="column.key === 'learnProgress'">
                <div class="flex gap-8 items-center cursor-pointer" data-toggle="modal" data-target="#modal"
                     :data-url="`/course_set/${courseSetId}/manage/course/${courseId}/students/${record.user.id}/process`">
                  <div class="flex flex-col justify-center items-start h-16 w-100 p-2 rounded-99 bg-[#F5F5F5]">
                    <div class="h-12 rounded-99" :style="`width: ${record.learningProgressPercent}px;`"
                         style="background-image: linear-gradient(90deg, rgba(0, 194, 97, 0.4), rgb(0, 194, 97));"></div>
                  </div>
                  <div class="flex items-center gap-4">
                    <span class="text-[#37393D] hover:text-[--primary-color] text-14 font-normal leading-22">{{
                        record.learningProgressPercent
                      }}%</span>
              ...
              <template v-else-if="column.key === 'operation'">
                <div class="flex items-center gap-16">
                  <div v-if="isAdmin || isTeacher"
                       ...
                       data-target="#modal"
                       :data-url="`/course_set/${courseSetId}/manage/course/${courseId}/student/deadline?ids=${record.user.id}`">
                    修改有效期
                  </div>
                  ...
                  <a-dropdown v-if="isNormalCourse" placement="bottomRight" trigger="['click']">
                    ...
                    <template #overlay>
                      <a-menu>
                        <a-menu-item v-if="isNormalCourse && record.user.canSendMessage" data-toggle="modal"
                                     data-target="#modal" :data-url="`/message/create/${record.user.id}`">
                          发私信
                        </a-menu-item>
                        <a-menu-item v-if="isAdmin" data-toggle="modal" data-target="#modal"
                                     :data-url="`/course_set/${courseSetId}/manage/course/${courseId}/students/${record.user.id}/show`">
                          查看资料
                        </a-menu-item>
                        <a-menu-item data-toggle="modal" data-target="#modal"
                                     :data-url="`/course_set/${courseSetId}/manage/course/${courseId}/student/${record.user.id}/remark`">
                          备注
                        </a-menu-item>
